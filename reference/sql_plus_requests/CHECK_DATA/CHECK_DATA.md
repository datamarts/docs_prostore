...
Проверка может давать ложноположительный результат. Этому подвержены оба варианта проверок: со списком столбцов и 
...
Параметры:
* `db_name` (опциональный) — имя логической базы данных, которой принадлежит проверяемая логическая таблица. Опционально, 
...
* `square_bracketed_column_list` (опциональный) — список проверяемых столбцов таблицы. Элементы списка указываются 
...
контрольная сумма дельты 1 в ADB отличается от контрольной суммы в других СУБД.

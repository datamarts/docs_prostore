...
Установка запрета для логической базы данных `sales` без кода:

```sql
DENY_CHANGES(sales)
```

Установка запрета для логической базы данных `sales` с кодом:

```sql
DENY_CHANGES(sales, 'my awesome code')
```

Установка запрета для логической БД, выбранной по умолчанию, без кода:

```sql
DENY_CHANGES()
```

Установка запрета для логической БД, выбранной по умолчанию, с кодом:

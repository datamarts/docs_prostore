...
[хранилища](../../../overview/main_concepts/data_storage/data_storage.md): указанная в запросе или, если такая не указана, 
[наиболее оптимальная СУБД](../../../working_with_system/data_reading/routing/routing.md) для исполнения 
[SELECT](../SELECT/SELECT.md)-подзапроса.

Вставка данных возможна при выполнении любого из условий:
* данные целевой таблицы размещены в одной СУБД хранилища, и эта же СУБД является источником данных;
* данные целевой таблицы размещены в ADB и (или) ADQM, и ADB является источником данных.
...
* в физические таблицы тех СУБД хранилища, в которых размещены данные указанной логической таблицы;
...
* `query` — [SELECT](../SELECT/SELECT.md)-подзапрос для выбора данных. Если в подзапросе указано ключевое слово 
  `DATASOURCE_TYPE` с псевдонимом СУБД хранилища, данные выбираются из этой СУБД, иначе — из СУБД, 

...
Снятие запрета изменений для логической базы данных `sales` без кода:

```sql
ALLOW_CHANGES(sales)
```

Снятие запрета изменений для логической базы данных `sales` с кодом:

```sql
ALLOW_CHANGES(sales, 'my awesome code')
```

Снятие запрета изменений для логической БД, выбранной по умолчанию, без кода:

```sql
ALLOW_CHANGES()
```

Снятие запрета изменений для логической БД, выбранной по умолчанию, с кодом:
